...
function getAnalysisPromptForType(siteType: WebsiteTypeInfo): string {
  const baseStructure = `
ABSOLUTE RULE — ALT TEXT CONTAINMENT: Alt text findings (missing, incomplete, or present) MUST appear ONLY in the SEO section. This rule has NO exceptions. Do NOT reference alt text in Performance, Design, Mobile, or any other section — not as a primary finding, not as a secondary point, not as supporting context. If your analysis of images, scripts, or performance signals would naturally surface alt text as a related issue, suppress it entirely in that section. The only permitted location for any alt text finding is SEO & Local Search.
