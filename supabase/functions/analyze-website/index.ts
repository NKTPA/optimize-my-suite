...
// ============================================
type EnvironmentType = "production_custom_domain" | "production_subdomain" | "deployment_preview" | "localhost_private";
...
function calculateDualScores(analysisResult: Record<string, unknown>, url: string): DualScore {
  const environment = detectEnvironment(url);

  // Weight multipliers based on environment
  const qualityWeight = environment.isPreview ? 0.85 : 0.7;
  const readinessWeight = environment.isPreview ? 0.15 : 0.3;
  ...
  const websiteQualityScore = Math.round(conversionScore * 0.6 + messagingScore * 0.25 + trustUxCombined * 0.15);
  ...
  const analyticsData = (analysisResult as Record<string, unknown>).analyticsDetected as
    | Record<string, unknown>
    | undefined;
  let analyticsScore: number;
  if (analyticsData) {
    const signals = [
      analyticsData.hasGoogleAnalytics,
      analyticsData.hasFacebookPixel,
      analyticsData.hasHotjarOrHeatmap,
  ...
  const sdData = (analysisResult as Record<string, unknown>).structuredDataDetected as
    | Record<string, unknown>
    | undefined;
  ...
  const productionReadinessScore = Math.round(
    indexingScore * 0.35 + performanceScore * 0.25 + analyticsScore * 0.2 + structuredDataScore * 0.2,
  );

  // Calculate overall score based on environment weights
  const overallScore = Math.round(websiteQualityScore * qualityWeight + productionReadinessScore * readinessWeight);
...
const LOVABLE_PLACEHOLDER_KEYWORDS = ["authenticating", "get started", "build software products", "lovable"];
...
function checkContentSufficiency(html: string, url: string, httpStatus?: number): NotScorableResult | null {
  ...
  if (detectLovablePlaceholderPage(html, url)) {
    return {
      ...
      reasonDisplay:
        "This URL shows a Lovable authentication or placeholder page, not the actual website content. The site may require login or may not be published yet.",
      finalUrl: url,
      httpStatus,
      htmlSizeKb,
      fixInstructions: [
  ...
  if (detectAgeVerificationPage(html)) {
    return {
      ...
      reasonDisplay:
        "This website has an age verification gate that prevents our analyzer from accessing the main content. Age-gated sites (alcohol, cannabis, gambling, adult content) cannot be scored without manual verification.",
      finalUrl: url,
      httpStatus,
      htmlSizeKb,
      fixInstructions: [
  ...
  if (htmlSizeBytes < MIN_HTML_SIZE_BYTES) {
    return {
      ...
      reasonDisplay:
        "The page returned too little HTML content to analyze. This may indicate a JavaScript-only shell, a redirect, or a blocked page.",
      finalUrl: url,
      httpStatus,
      htmlSizeKb,
      fixInstructions: [
  ...
  if (bodyText.length < MIN_BODY_TEXT_LENGTH) {
    ...
    return {
      ...
      reasonDisplay:
        "The page contains very little readable text content. It may be a JavaScript-only application that requires client-side rendering.",
      finalUrl: url,
      httpStatus,
      htmlSizeKb,
      fixInstructions: [
...
function extractDataFromHtml(html: string, url: string, rawHtml?: string) {
  ...
  const getMetaContent = (name: string) => {
    ...
    for (const source of [headSource, html]) {
      const match =
        source.match(new RegExp(`<meta[^>]*(?:name|property)=["']${name}["'][^>]*content=["']([^"']*)["']`, "i")) ||
  ...
  const ctaPatterns =
    /(?:call|book|schedule|contact|quote|estimate|get started|free|request)[^<]*(?:<\/(?:a|button)>)/gi;
  const ctaMatches = html.match(ctaPatterns) || [];
  const ctaButtons = ctaMatches
  ...
  const hasViewport =
    /<meta[^>]*name=["']viewport["']/i.test(headSource) || /<meta[^>]*name=["']viewport["']/i.test(html);
  const hasFavicon =
    /<link[^>]*rel=["'](?:icon|shortcut icon|apple-touch-icon)["']/i.test(headSource) ||
  ...
  return {
    ...
    // ── Real analytics / structured-data detection ──
    analytics: {
      hasGoogleAnalytics: /google-analytics\.com|googletagmanager\.com|gtag\(|ga\(\s*['"]send|UA-\d|G-[A-Z0-9]/i.test(
        html,
      ),
      hasFacebookPixel: /connect\.facebook\.net\/.*fbevents\.js|fbq\(\s*['"]init/i.test(html),
      hasHotjarOrHeatmap: /hotjar\.com|static\.hotjar\.com|clarity\.ms\/tag/i.test(html),
      rawSignals: [...(html.match(/googletagmanager\.com\/gtag|google-analytics\.com\/analytics/gi) || [])].slice(0, 3),
    },
    structuredData: {
      hasJsonLd:
        /<script[^>]*type=["']application\/ld\+json["']/i.test(headSource) ||
        /<script[^>]*type=["']application\/ld\+json["']/i.test(html),
      hasOpenGraph: /<meta[^>]*property=["']og:/i.test(headSource) || /<meta[^>]*property=["']og:/i.test(html),
      hasTwitterCard:
        /<meta[^>]*name=["']twitter:card["']/i.test(headSource) || /<meta[^>]*name=["']twitter:card["']/i.test(html),
      schemaTypes: (() => {
        const matches = html.match(/["']@type["']\s*:\s*["']([^"']+)["']/gi) || [];
        return [
          ...new Set(
            matches
              .map((m) => {
...
function detectWebsiteType(
  ...
  if (lowerHtml.includes("shopify") || lowerHtml.includes("woocommerce") || lowerHtml.includes("bigcommerce"))
    ecomScore += 3;
  ...
  try {
    ...
    if (!isOwner) {
      ...
      if (isWorkspaceError(workspaceResult)) {
        ...
        return new Response(JSON.stringify({ error: workspaceResult.error }), {
          ...
          try {
            ...
            if (fcResponse.ok) {
              ...
              if (typeof fcHtml === "string" && fcHtml.length > 500) {
                ...
                logStep("Firecrawl fallback returned insufficient HTML", {
                  length: typeof fcHtml === "string" ? fcHtml.length : 0,
                });
              }
            } else {
            ...
            logStep("Firecrawl fallback error", {
              error: fcError instanceof Error ? fcError.message : String(fcError),
            });
          }
        } else {
      ...
      if (!html) {
        ...
        const notScorableResult = {
          ...
          notScorable: {
            ...
            reasonDisplay:
              "Could not access this website. The site may have bot protection enabled, require authentication, or be temporarily unavailable.",
            finalUrl: url,
            httpStatus: 0,
            htmlSizeKb: 0,
            fixInstructions: [
        ...
        try {
          ...
          if (fcResponse.ok) {
            ...
            if (typeof fcHtml === "string" && fcHtml.length > 500) {
              ...
              logStep("Firecrawl JS-shell fallback returned insufficient HTML", {
                length: typeof fcHtml === "string" ? fcHtml.length : 0,
              });
            }
          } else {
          ...
          logStep("Firecrawl JS-shell fallback error", {
            error: fcError instanceof Error ? fcError.message : String(fcError),
          });
        }
      } else {
    ...
    if (false && firecrawlKeyForScreenshot && !isManualHtml) {
      try {
        ...
        const screenshotResponse = await fetch("https://api.firecrawl.dev/v1/scrape", {
          method: "POST",
          headers: {
            Authorization: `Bearer ${firecrawlKeyForScreenshot}`,
            "Content-Type": "application/json",
          },
          body: JSON.stringify({
            url,
        ...
        if (screenshotResponse.ok) {
          ...
          const ssUrl: string | undefined =
            ssData?.data?.screenshot || ssData?.data?.screenshotUrl || ssData?.screenshot || ssData?.screenshotUrl;

          // Also handle base64 returned directly
          const ssBase64Direct: string | undefined = ssData?.data?.screenshotBase64 || ssData?.screenshotBase64;

          if (ssBase64Direct) {
            screenshotBase64 = ssBase64Direct;
            logStep("Screenshot captured (direct base64)", { base64Len: screenshotBase64.length });
          } else if (ssUrl) {
            // Fetch the image and convert to base64 so we can pass it inline
            try {
              const imgRes = await fetch(ssUrl);
              if (imgRes.ok) {
                const imgBuffer = await imgRes.arrayBuffer();
                // Cap at 3MB to avoid gateway payload limits
                const cappedBuffer = imgBuffer.byteLength > 3_000_000 ? imgBuffer.slice(0, 3_000_000) : imgBuffer;
                const bytes = new Uint8Array(cappedBuffer);
                let binary = "";
                for (let i = 0; i < bytes.byteLength; i++) binary += String.fromCharCode(bytes[i]);
                screenshotBase64 = btoa(binary);
                logStep("Screenshot captured", { base64Len: screenshotBase64.length, fromUrl: ssUrl.slice(0, 60) });
              } else {
                logStep("Screenshot image fetch failed", { status: imgRes.status });
              }
            } catch (imgErr) {
              logStep("Screenshot image fetch error", {
                error: imgErr instanceof Error ? imgErr.message : String(imgErr),
              });
            }
          } else {
            logStep("Screenshot: no URL or base64 in Firecrawl response", {
              keys: Object.keys(ssData?.data || ssData || {}).join(","),
            });
          }
        } else {
          const ssErrText = await screenshotResponse.text().catch(() => "");
          logStep("Screenshot capture failed", { status: screenshotResponse.status, body: ssErrText.slice(0, 200) });
        }
      } catch (ssErr) {
        logStep("Screenshot capture error (non-fatal)", {
    ...
    // Build the user message — include screenshot image block when available
    const textBlock = `Analyze this website: ${url}${environmentContext}

Website Type: ${websiteType.displayName} (${websiteType.confidence} confidence)

Extracted Data (HTML metadata, text, CTAs, forms, analytics signals, structured data):
${JSON.stringify(extractedData, null, 2)}

${screenshotBase64 ? "A full-page screenshot of the website is attached as the image above. Use it to assess visual design quality, layout hierarchy, above-the-fold content, CTA placement, mobile readiness signals, trust indicators, and overall professionalism. Factor what you SEE heavily into your designUx, mobile, and trust scores." : "No screenshot available — base visual scores on HTML signals only."}
...
Provide a comprehensive analysis with specific, actionable recommendations appropriate for this ${websiteType.displayName}. Return ONLY valid JSON matching the specified structure.`;
    ...
    try {
      ...
      logStep("ERROR: JSON parse failed", {
    ...
    // Attach real analytics & structured data signals so calculateDualScores can use them
    analysisResult.analyticsDetected = extractedData.analytics;
    analysisResult.structuredDataDetected = extractedData.structuredData;
    // Flag whether a screenshot was used for this analysis
    analysisResult.screenshotUsed = !!screenshotBase64;
