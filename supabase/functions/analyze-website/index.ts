...
function getAnalysisPromptForType(siteType: WebsiteTypeInfo): string {
  const baseStructure = `
ABSOLUTE RULE — ALT TEXT CONTAINMENT: Alt text findings MUST appear ONLY in the SEO section. This means NEVER in Performance, NEVER in Design, NEVER in Mobile, NEVER in any other section. This includes any sentence where alt text appears alongside image optimization, file size, or script analysis. If writing about images in the Performance section, discuss only file size, script count, and load time. Do not mention alt text in that context under any circumstances.
